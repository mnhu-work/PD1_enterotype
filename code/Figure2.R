#########Fungi alpha diversity from microeco
alpha <- read.delim("../data/Fungi_alpha_div_microeco_output.tsv",row.names=1)
meta <- read.csv("../data/clustering_results.csv",row.names = 1)
...
library(ggpubr)
##plots
ggplot(df, aes(group, Fisher, fill = group)) +
  ...
  labs(x = "Group", y = "Alpha diversity (Fisher)") +
  scale_fill_manual(values = colset.d.2)
ggsave(file = "../../rarefying_596samples/Fisher_Fungi_microeco.pdf",height = 4,width = 2.5)


###########
phylum_abu <- read.delim("../data/Fungi_Phylum_abu.tsv",row.names=1)
Phylum <- phylum_abu
#rowSums(Phylum[,c(1:7)])
mat <- Phylum[,c(1:7)]/100
...
############plot phylumn
###
col.hm <- c(colset.d.12,colset.d.4)
...
ggsave(filename = "../../rarefying_596samples/Fungi_Phylum_percentage_barplot.pdf",width = 4,height = 6)
...
B_A_ratio <- phylum_abu$B_A_ratio
phylum_abu$logB_A_ratio <- log2(B_A_ratio+1)
#pdf(file = "plots/Fungi_Basidiomycota.pdf",height = 4,width = 2.5)
ggplot(phylum_abu, aes(group, logB_A_ratio, fill = group)) +
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.position = "none",
        axis.ticks = element_line(color = "black", linewidth = 1)) +
  labs(x = "Group", y = "logB_A_ratio") +
  scale_fill_manual(values = colset.d.2)
ggsave(filename = "../../rarefying_596samples/logB_A_ratio.pdf",height = 4,width = 2.5)